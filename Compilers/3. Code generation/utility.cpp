...
struct VariableInfo
{
    char* name;
    int memloc;
    //adding value attribute initially set to zero
    int value=0;
...
struct SymbolTable
{
    ...
    // This function takes the variable name 'var' and its new value 'newValue'.
    void updateVarValue(const char* var, int newValue) {
        // Iterate through the symbol hash table.
        for (int i = 0; i < SYMBOL_HASH_SIZE; i++) {
            // Get the first VariableInfo in the current hash bucket.
            VariableInfo* info = var_info[i];

            // Traverse the linked list of variables in the current hash bucket.
            while (info) {
                // Check if the current variable name matches the target variable.
                if (Equals(var, info->name)) {
                    // Update the value of the variable to the new value.
                    info->value = newValue;

                    // Update the head of the linked list in the hash bucket
                    // (optional in some implementations, depends on requirements).
                    var_info[i] = info;

                    // Exit the function since the variable has been found and updated.
                    return;
                }

                // Move to the next VariableInfo in the linked list.
                info = info->next_var;
            }
        }

        // If the function reaches this point, the variable was not found in the symbol table.
        // Depending on requirements, you might want to handle this case (e.g., report an error).
    }
    void Print()
    {
        int i;
